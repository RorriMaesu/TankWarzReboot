import { Vector2D, GameState, GameConfig, Weapon, WeaponType, Crate, Mine } from './types.js';
...
export class GameEngine {
  ...
  private windParticles: Vector2D[] = [];
  private floatingTexts: FloatingText[] = [];
  private shockwaves: Shockwave[] = [];
  private nanites: Nanite[] = [];
  ...
  private gameMode: 'vs_ai' | 'pvp' = 'vs_ai';
  private shakeIntensity: number = 0;
  private screenFlash: number = 0;
  private turnsElapsed: number = 0;

  // Tactical Evasion Target for AI
  private aiMoveTargetX: number | null = null;
  private lastLandingX: number | null = null;

  // Burst fire state
  private burstShotsRemaining: number = 0;
  private burstTimer: number = 0;
  private burstDelay: number = 0;
  private burstPower: number = 0;
  private burstAngle: number = 0;
  private burstWeapon: Weapon | null = null;
  private burstOwner: 'player' | 'ai' = 'player';

  // Nuke unlock state
  private player1HasNuke: boolean = false;
  private player2HasNuke: boolean = false;
  ...
  private initGame(seed?: number) {
    ...
    p1.weapon = { ...this.weapons.small_cannon };
    ...
    p2.weapon = { ...this.weapons.small_cannon };
    ...
    const nukeOpt = document.getElementById('opt-nuke') as HTMLOptionElement;
    if (nukeOpt) nukeOpt.disabled = true;
  ...
  private setupEventListeners() {
    ...
    powerSlider?.addEventListener('input', () => this.updateSliderOutputs());
    angleSlider?.addEventListener('input', () => this.updateSliderOutputs());

    // Weapon selection
    const weaponSelector = document.getElementById('weapon-selector') as HTMLSelectElement;
    weaponSelector?.addEventListener('change', (e) => {
      const type = (e.target as HTMLSelectElement).value as WeaponType;
      const activePlayer = this.getActivePlayer();
      if (activePlayer && this.weapons[type]) {
        activePlayer.weapon = { ...this.weapons[type] };
        this.uiManager.logMessage(`Selected Weapon: ${activePlayer.weapon.name}`);
      }
    });

    // Difficulty selection
    const diffSelector = document.getElementById('difficulty-selector') as HTMLSelectElement;
    diffSelector?.addEventListener('change', (e) => {
      this.difficulty = (e.target as HTMLSelectElement).value as 'easy' | 'medium' | 'expert';
    ...
    modeSelector?.addEventListener('change', (e) => {
      this.gameMode = (e.target as HTMLSelectElement).value as 'vs_ai' | 'pvp';
    ...
    window.addEventListener('keydown', (e) => {
      if (!this.isMyTurnLocal()) return;

      const player = this.getActivePlayer();
      if (!player || player.fuel <= 0) return;
  ...
  private getDynamicFuelCost(weaponType: WeaponType, power: number): number {
  ...
  private updateWeaponDropdownCosts(power: number) {
    const activePlayer = this.getActivePlayer();
    const select = document.getElementById('weapon-selector') as HTMLSelectElement;
    if (!select) return;

    for (let i = 0; i < select.options.length; i++) {
      const option = select.options[i];
      const type = option.value as WeaponType;
  ...
  private updateSliderOutputs() {
    const power = parseInt((document.getElementById('power-slider') as HTMLInputElement)?.value || '70');
    const angle = parseInt((document.getElementById('angle-slider') as HTMLInputElement)?.value || '45');
  ...
  private synchronizeSliders(player: Player) {
  ...
  private updateInputControlsAvailability() {
    const myTurn = this.isMyTurnLocal() && this.state !== 'PROJECTILE_FLIGHT' && this.state !== 'GAME_OVER';
    
    const powerSlider = document.getElementById('power-slider') as HTMLInputElement;
    const angleSlider = document.getElementById('angle-slider') as HTMLInputElement;
    const fireBtn = document.getElementById('fire-button') as HTMLButtonElement;
    const weaponSelector = document.getElementById('weapon-selector') as HTMLSelectElement;

    if (powerSlider) powerSlider.disabled = !myTurn;
    if (angleSlider) angleSlider.disabled = !myTurn;
    if (fireBtn) fireBtn.disabled = !myTurn;
    if (weaponSelector) weaponSelector.disabled = !myTurn;
  }
  ...
  private fireSingleProjectile() {
    ...
    const projectile = new Projectile(
      ...
      spawnPos,
      velocity,
      this.burstWeapon.damage,
      this.burstWeapon.radius,
      this.burstWeapon.projectileRadius,
      this.burstWeapon.type
    );
    ...
    if (this.burstWeapon.type === 'nuke') {
      ...
      owner.weapon = { ...this.weapons.small_cannon };
      this.synchronizeSliders(owner);
    }
  }

  private triggerExplosion(x: number, y: number, radius: number, damage: number, owner: 'player' | 'ai', kineticMultiplier: number = 1.0) {
    const isDirtSpreader = this.burstWeapon?.type === 'dirt_spreader';
    ...
    } else if (this.burstWeapon?.type === 'bouncing_grenade') {
  ...
  private updatePhysics() {
    ...
    for (let i = this.crates.length - 1; i >= 0; i--) {
      ...
      for (const player of this.players) {
        const dist = Math.sqrt(
          Math.pow(crate.position.x - player.position.x, 2) +
    ...
    if (this.state === 'PROJECTILE_FLIGHT') {
      ...
      for (let i = this.projectiles.length - 1; i >= 0; i--) {
        ...
        for (let s = 0; s < subSteps; s++) {
          ...
          let hitTank = false;
          for (const player of this.players) {
            if (p.owner === player.type) continue; // no self-collision
          ...
          if (p.type === 'cluster_bomb' && !p.wasSplit) {
            ...
            if (p.velocity.y >= 0 && prevVy < 0) {
              ...
              for (let c = 0; c < 3; c++) {
                const subShell = new Projectile(
                  `proj-sub-${Date.now()}-${c}-${Math.random()}`,
                  p.owner,
                  { ...p.position },
                  { x: p.velocity.x + offsets[c], y: 1.0 }, // push slightly downwards
                  this.weapons.cluster_bomb.damage,
                  this.weapons.cluster_bomb.radius,
                  this.weapons.cluster_bomb.projectileRadius,
                  'cluster_sub'
                );
      ...
      if (!projectileStillActive) {
        if (this.burstShotsRemaining > 0) {
          ...
          if ((this.state as string) !== 'GAME_OVER') {
            const isMyTurnBeforeSwitch = (this.network?.role === 'host' && (this.state as string) === 'PLAYER_TURN') || 
                                         (this.network?.role === 'guest' && (this.state as string) === 'ENEMY_TURN') ||
                                         !this.isMultiplayer;
            ...
            // Shift turns
            if ((this.state as string) === 'PLAYER_TURN') {
  ...
  private draw() {
    ...
    this.renderer.drawPlayers(this.players, activePlayerType as any);
  ...
  private drawAimGuide(ctx: CanvasRenderingContext2D) {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      if (pos.y >= this.terrain.getHeight(pos.x)) break;
      if (pos.x < 0 || pos.x > this.config.canvasWidth) break;
    }
    ctx.restore();
  }

  private drawWindParticles(ctx: CanvasRenderingContext2D) {
    ctx.save();
  ...
  private drawCrates(ctx: CanvasRenderingContext2D) {
    ctx.save();
  ...
  private setupLobbyHandlers() {
    ...
    this.network.onFire((power, angle, weaponType) => {
      ...
      activePlayer.weapon = { ...this.weapons[weaponType] };
    ...
    quickMatchBtn?.addEventListener('click', async () => {
      ...
      if (lobbyStatusText) lobbyStatusText.innerText = "Connecting to matchmaking lobby...";
      
      const success = await this.network.init();
      if (!success) {
      ...
      if (lobbyStatusText) lobbyStatusText.innerText = "Searching for opponent...";
      
      await this.network.startQuickMatch((role) => {
        this.isMultiplayer = true;
        this.gameMode = 'pvp';
        if (role === 'guest') {
          if (lobbyStatusText) lobbyStatusText.innerText = "Joining host lobby...";
        } else {
          if (lobbyStatusText) lobbyStatusText.innerText = "Opponent found! Initializing battleground...";
        }
      });
    });
    ...
    createRoomBtn?.addEventListener('click', async () => {
      lobbyStatus?.classList.remove('hidden');
      if (lobbyStatusText) lobbyStatusText.innerText = "Provisioning private room...";
      ...
      const code = await this.network.hostPrivateRoom(() => {
        lobbyStatus?.classList.add('hidden');
        lobbyOverlay?.classList.add('hidden');
        this.isMultiplayer = true;
        this.gameMode = 'pvp';
    ...
    joinRoomBtn?.addEventListener('click', async () => {
      ...
      lobbyStatus?.classList.remove('hidden');
      if (lobbyStatusText) lobbyStatusText.innerText = `Connecting to room ${code}...`;
      ...
      await this.network.joinPrivateRoom(code);
    });
  ...
  private setupTouchControls() {
    ...
    canvas.addEventListener('touchstart', (e) => {
      ...
      const dist = Math.sqrt(
        Math.pow(touchX - player.position.x, 2) +
    ...
    canvas.addEventListener('touchmove', (e) => {
      ...
      if (dragAngle > Math.PI) dragAngle = Math.PI;
      ...
      const powerSlider = document.getElementById('power-slider') as HTMLInputElement;
      const angleSlider = document.getElementById('angle-slider') as HTMLInputElement;
      if (powerSlider) powerSlider.value = Math.round(dragPower).toString();
      if (angleSlider) angleSlider.value = Math.round(dragAngle * (180 / Math.PI)).toString();
      this.updateSliderOutputs();
    ...
    const startDriving = (direction: 'left' | 'right') => {
    ...
    leftBtn?.addEventListener('touchend', (e) => { e.preventDefault(); stopDriving(); });
