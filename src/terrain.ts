...
export class Terrain {
  ...
  private initializeHeights(seed?: number) {
   ...
   */
  public deform(x: number, y: number, radius: number) {
   ...
   */
  public addDirt(x: number, y: number, radius: number) {
    const left = Math.max(0, Math.floor(x - radius));
    const right = Math.min(this.width - 1, Math.floor(x + radius));
  ...
  public draw(ctx: CanvasRenderingContext2D, turnsElapsed: number = 0) {
