...
export class BackgroundManager {
  ...
  private distantImg: HTMLImageElement | null = null;
  private nearImg: HTMLImageElement | null = null;
  private assetsLoaded: { sky: boolean; distant: boolean; near: boolean } = { sky: false, distant: false, near: false };

  // Programmatic generation fallbacks (in case image files aren't generated yet)
  private distantBuildings: { x: number; width: number; height: number }[] = [];
  private nearBuildings: { x: number; width: number; height: number; lights: { y: number; color: string; offset: number }[] }[] = [];
  ...
  private loadImages() {
    ...
    this.skyImg.src = 'assets/sky.png?v=1.2.0';
    this.skyImg.onload = () => { 
    ...
    this.distantImg.src = 'assets/distant_ruins.png?v=1.2.0';
    ...
    this.nearImg.src = 'assets/near_ruins.png?v=1.2.0';
    this.nearImg.onload = () => { 
  ...
  private initBuildings() {
    ...
    while (currentX < this.config.canvasWidth + 100) {
      ...
      const lights: { y: number; color: string; offset: number }[] = [];
  ...
  public update(windSpeed: number) {
    ...
    this.ashParticles.forEach(p => {
      ...
      if (p.x < -10) p.x = this.config.canvasWidth + 10;
      else if (p.x > this.config.canvasWidth + 10) p.x = -10;
    });
  ...
  private drawTiledImage(ctx: CanvasRenderingContext2D, img: HTMLImageElement, parallaxOffset: number, yOffset: number, targetHeight: number) {
  ...
  public draw(ctx: CanvasRenderingContext2D, cameraOffsetX: number = 0) {
    const w = this.config.canvasWidth;
