import { Vector2D, GameConfig } from './types.js';
...
export class PhysicsEngine {
   ...
   */
  public updateProjectile(position: Vector2D, velocity: Vector2D, dt: number = 1): Vector2D {
   ...
   */
  public simulateTrajectory(initialPosition: Vector2D, power: number, angle: number, terrain: Terrain, speedMultiplier: number = 0.15): Vector2D {
    const resolutionScale = this.config.canvasWidth / 1024;
