export class UI {
    ...
    update(players, state) {
        ...
        if (player) {
            ...
            if (hpEl)
                hpEl.textContent = Math.floor(player.health).toString();
            const fuelEl = document.getElementById('player-fuel');
            if (fuelEl)
                fuelEl.textContent = Math.floor(player.fuel).toString();
        }
        const ai = players.find(p => p.type === 'ai');
        if (ai) {
            const hpEl = document.getElementById('ai-hp');
            if (hpEl)
                hpEl.textContent = Math.floor(ai.health).toString();
            const fuelEl = document.getElementById('ai-fuel');
            if (fuelEl)
                fuelEl.textContent = Math.floor(ai.fuel).toString();
        }
