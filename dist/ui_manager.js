export class UIManager {
    updateUI(players, state, wind, gameMode) {
        const player = players.find(p => p.type === 'player');
        if (player) {
            const hpEl = document.getElementById('player-hp');
            if (hpEl)
                hpEl.textContent = Math.max(0, Math.floor(player.health)).toString();
            const fuelEl = document.getElementById('player-fuel');
            if (fuelEl)
                fuelEl.textContent = Math.max(0, Math.floor(player.fuel)).toString();
            // Update bars
            const hpBar = document.getElementById('player-hp-bar');
            if (hpBar)
                hpBar.style.width = `${Math.max(0, (player.health / 100) * 100)}%`;
            const fuelBar = document.getElementById('player-fuel-bar');
            if (fuelBar)
        ...
        if (ai) {
            ...
            if (hpEl)
                hpEl.textContent = Math.max(0, Math.floor(ai.health)).toString();
            const fuelEl = document.getElementById('ai-fuel');
            if (fuelEl)
                fuelEl.textContent = Math.max(0, Math.floor(ai.fuel)).toString();
            // Update bars
            const hpBar = document.getElementById('ai-hp-bar');
            if (hpBar)
                hpBar.style.width = `${Math.max(0, (ai.health / 100) * 100)}%`;
            const fuelBar = document.getElementById('ai-fuel-bar');
            if (fuelBar)
                fuelBar.style.width = `${Math.max(0, (ai.fuel / ai.maxFuel) * 100)}%`;
        }
    ...
    updateSliders(power, angle) {
        const powerVal = document.getElementById('power-val');
        if (powerVal)
            powerVal.textContent = power.toString();
